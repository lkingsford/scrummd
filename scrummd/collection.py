...
Groups = OrderedDict[Optional[str | Field], GroupValue]
...
def group_collection(
    ...
    if predefined:
        ...
        for predefined_field in [typed_field(f.casefold()) for f in group]:
            card_groups[predefined_field] = GroupValue(Groups(), Collection())
    else:
        ...
        for ordered_field in ordered_fields:
            card_groups[ordered_field] = GroupValue(Groups(), Collection())

    card_groups[None] = GroupValue(Groups(), Collection())
